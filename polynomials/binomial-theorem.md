...
Let $$x = 3a^2$$, $$y = -7b$$ and $$n = 6$$. Then:

$$
\begin{align}
(3a^2 - 7b)^6 \\
&= {6 \choose 0} (3a^2)^6 + {6 \choose 1}(3a^2)^5(-7b) + ... + {6 \choose 5}(3a^2)(-7b)^5 + {6 \choose 6}(-7b)^6 \\
&= 729a^{12} - 10206 a^{10}b + 59535 a^8b^2 - 185220 a^6b^3 + 324135a^4b^4 - 302526 a^2b^5 + 117649b^6
\end{align}
$$
...
## The General Term
We define the $$k$$-th term in the expansion of a binomial as $$t_k = {n \choose k} x^{n-k}y^k$$. 

In the expansion of $$(3a^2 - 7b)^6$$ from the previous section, the general term is:

$$
\begin{align}
t_k &= {6 \choose k} (3a^2)^{6 - k} (-7b)^k \\ 
&= {6 \choose k} 3^{6-k} a^{12 - 2k} (-1)^k 7^k b^k \\ 
&= (-1)^k {6 \choose k} 3^{6-k}7^k a^{12 - 2k}b^k
\end{align}
$$

You may be asking yourself why we've written the general term this way. With the $$(-1)^k$$ at the front, we can quickly tell whether a term is positive or negative. By splitting the $$3$$, $$7$$, $$a$$ and $$b$$, each exponential base is isolated, simplifying the multiplication process (i.e. we don't need to multiply $$3$$ and $$a^2$$ and then raise that to an exponent; instead, we raise $$3$$ and $$a^2$$ to exponents separately, as we are more likely to know the powers of $$3$$ on their own).
...
$$
\begin{align}
t_k &= {7 \choose k} (x^5)^{7-k}(-x^{-2})^{k} \\
&= (-1)^k {7 \choose k} x^{35 - 5k} x^{-2k} \\
&= (-1)^k {7 \choose k} x^{35 - 7k}
\end{align}
$$
...
$$f(x) = (2x - 3x^2)^4$$
$$= {4 \choose 0}(2x)^4 + {4 \choose 1}(2x)^3(-3x^2) + {4 \choose 2}(2x)^2(-3x^2)^2 + {4 \choose 3}(2x)(-3x^2)^3 + {4 \choose 4}(-3x^2)^4$$
$$= 16x^4 - 96x^5 + 216x^6 - 216x^7 + 81x^8$$
$$\implies f(1) = 16 - 96 + 216 - 216 + 81 = -80 + 81 = 1$$
...
An interesting result in the above example is that no matter how large $$n$$ is, the sum of the coefficients in $$(2x - 3x^2)^n$$ is always either $$1$$ or $$-1$$, depending on whether $$n$$ is even or odd. This is interesting, as when $$n = 1000$$, terms of the form $${1000 \choose I}$$ can be on the order of $$10^{30}$$, yet the sum of coefficients is still just $$1$$ ---- fascinating!
