<title>5.4: Factor and Remainder Theorems, Complex Roots – Polynomials</title>

# 5.4: Polynomial Division, Factor and Remainder Theorems, Complex Roots
---
...
Watching the embedded video will make this process far more clear. Again, practicing synthetic division is the only way you will truly understand how to use the tool.
...
Now, we're equipped to do some analysis regarding the division algorithm. Suppose we're dividing some arbitrary polynomial $$p(x)$$ by linear divisor $$x-a$$. Then, we can say
...
> ### Definition: **Remainder theorem** </a>
> If $$p(x)$$ is a polynomial, such that $$r(x)$$ is the remainder that results from dividing $$p(x)$$ by $$x - a$$, then $$p(a) = r(a)$$.
...
Notice that this only applies to polynomial divisions with linear divisors. 
...
> ### Definition: **Factor theorem** </a>
> The factor theorem states that $$x-a$$ is a factor of $$p(x)$$ if and only if $$p(a) = 0$$. 

<br>

Clearly, $$x - 7$$ is not a factor of $$p(x) = x^2 - 5x + 6$$, since the remainder is 20 (or, equivalently, $$p(7) = 20 \neq 0$$). However, $$x - 3$$ is a factor of $$p(x)$$, as $$x^2 - 5x + 6 = (x-3) (x-2) + 0$$ (or, equivalently, $$p(3) = 0$$). 

As another example, since we can factor $$p(x) = x^3 + 7x^2 + 6x$$ into $$p(x) = x(x + 1)(x + 6)$$, we know that $$p(0) = p(-1) = p(-6) = 0$$.
